...
class BinarySearchTree:
	...
	def __searchItem(self, tNode:TreeNode, x) -> TreeNode:
		# TODO: 아래 4가지 경우를 재귀로 구현하고 TreeNode를 반환하라.
		#   1) tNode가 None         → None 반환 (탐색 실패)
		#   2) x == tNode.item      → tNode 반환 (탐색 성공)
		#   3) x < tNode.item       → 왼쪽 서브트리로 재귀
		#   4) x > tNode.item       → 오른쪽 서브트리로 재귀
		if tNode == None :
			return None
		elif x == tNode.item:
			return tNode
	...
	def __insertItem(self, tNode:TreeNode, newItem) -> TreeNode:
		# TODO: 아래 3가지 경우를 재귀로 구현하고 tNode를 반환하라.
		#   1) tNode가 None         → TreeNode(newItem, None, None) 생성 후 반환
		#   2) newItem < tNode.item → 왼쪽 서브트리로 재귀, 결과를 tNode.left에 연결
		#   3) newItem >= tNode.item→ 오른쪽 서브트리로 재귀, 결과를 tNode.right에 연결
		if tNode == None :
